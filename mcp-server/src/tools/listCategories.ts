...
const inputSchema = {
  primary: z
    .string()
    .optional()
    .describe("Filter by primary category (e.g. FOOD_AND_DRINK, TRAVEL)"),
};
...
export function register(server: McpServer): void {
  server.tool(
    ...
    async (args) => {
      if (args.primary) {
        const key = args.primary.toUpperCase();
        const detailed = CATEGORIES[key];
        if (!detailed) {
          const available = Object.keys(CATEGORIES).join(", ");
          return textResponse(
            `Unknown primary category: "${args.primary}". Available: ${available}`,
          );
        }
        return textResponse(
          JSON.stringify({ primary: key, detailed }, null, 2),
        );
      }

      const summary = Object.entries(CATEGORIES).map(([primary, detailed]) => ({
        primary,
